...
The refactor described in `doc/refactor_phase_plan.md` is partially complete.
...
### Remaining sections (in priority order per `refactor_phase_plan.md`)
...
Per `doc/refactor_phase_plan.md`, once all scenarios pass, collapse the 7-stage pipeline:
...
The golden rule should almost never fire during normal operation. If it fires during tests, it indicates a bug in a phase handler. At that point, delete all the now-unreachable pipeline stages (`start_rehoming_logic`, `start_servicing_request_logic`, `start_moving_logic`, `stop_moving_logic`, `complete_servicing_request_logic`, `enforce_safety_overrides`).
