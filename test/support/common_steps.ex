defmodule Elevator.CommonSteps do
  ...
  defgiven ~r/^(?:the (?:elevator )?)?is (?:in )?"(?<status>[^"]+)" (?:and doors are|at floor (?<floor1>\d+) with doors) "(?<door>:[^"]+)"(?: at floor (?<floor2>\d+))?$/,
           %{status: status, door: door} = params,
           state do
    {k, v} = parse_state_field(status)
    floor = params["floor1"] || params["floor2"]
    new_state = %{state.state | k => v, door_status: parse_atom(door)}

    new_state =
      if floor, do: %{new_state | current_floor: String.to_integer(floor)}, else: new_state

    {:ok, %{state | state: new_state}}
  end

  defgiven ~r/^(?:the (?:elevator )?)?is "(?<status>:[^"]+)" and doors are "(?<door>:[^"]+)"$/,
           %{status: status, door: door},
           state do
    new_state = %{state.state | heading: parse_atom(status), door_status: parse_atom(door)}
    {:ok, %{state | state: new_state}}
  end
  ...
  defgiven ~r/^(?:the (?:elevator )?)?is in "(?<field1>[^"]+)" with "(?<field2>[^"]+)"$/,
           %{field1: f1, field2: f2},
           state do
  ...
  defgiven ~r/^(?:the (?:elevator )?)?is in "(?<field1>[^"]+)" with "(?<field2>[^"]+)" and "(?<field3>[^"]+)" is "(?<val3>[^"]+)"$/,
           %{field1: f1, field2: f2, field3: f3, val3: v3},
           state do
    {k1, v1} = parse_state_field(f1)
    {k2, v2} = parse_state_field(f2)
  ...
  defgiven ~r/^(?:the (?:elevator )?)?is in "(?<field>[^"]+)" with no pending requests$/,
           %{field: field},
           state do
    {k, v} = parse_state_field(field)
  ...
  defgiven ~r/^(?:the (?:elevator )?)?is in "(?<field>[^"]+)" at floor (?<floor>\d+)$/,
    ...
    new_state =
      Map.merge(
        state.state,
        Map.new([{k, v}, {:current_floor, String.to_integer(floor)}, {:heading, :up}])
      )
  ...
  defwhen ~r/^(?:a|the) "(?<event>:[^"]+)"(?: confirmation| message)? is received$/,
          %{event: event},
          state do
  ...
  defwhen ~r/^(?:a (?:new )?)?request for floor (?<floor>\d+) is received$/,
          %{floor: floor},
          state do
  ...
  defwhen ~r/^(?<seconds>\d+) seconds pass without activity \("(?<event>:[^"]+)" event\)$/,
          %{seconds: seconds, event: event},
          state do
    # Simulate time passing by sending a door timeout or other timer event with a timestamp offset
    {new_state, actions} =
      Core.handle_event(state.state, parse_atom(event), String.to_integer(seconds) * 1000)
  ...
  defthen ~r/^(?:a )?"(?<value>[^"]+)" request should be (?:automatically )?added(?: to the queue)?$/,
          %{value: value},
          state do
  ...
  defthen ~r/^(?:the (?:elevator )?)?(?:the )?"(?<field>[^"]+)" (?:queue )?(?:should )?(?:becomes?|reverts? to|stays?|remains?|transitions? back to) "(?<value>[^"]+)"$/,
  ...
  defthen ~r/^(?:the )?(?<target>motor|door) should receive (?:a|an) "(?<command>:[^"]+)" command$/,
  ...
  defthen ~r/^(?:the )?request (?:for the current floor )?should be (?:removed from|consumed by|immediately fulfilled)(?: the (?:queue|requests))?$/,
          _vars,
          state do
  ...
  defthen ~r/^(?:the (?:elevator )?)?"(?<field>[^"]+)" should remain "(?<value>:[^"]+)" until a new direction is chosen$/,
          %{field: field, value: value},
          state do
  ...
  defwhen ~r/^(?<val>\d+) minutes \((?<sec>\d+)s\) pass without any activity$/,
          %{sec: sec},
          state do
    {new_state, actions} =
      Core.handle_event(state.state, :inactivity_timeout, String.to_integer(sec) * 1000)

    {:ok, %{state | state: new_state, actions: actions}}
  end

  defthen ~r/^(?:the (?:elevator )?)?should return to floor (?<floor>\d+)$/,
          %{floor: floor},
          state do
