defmodule Elevator.SafetyFeatureTest do
  ...
  defthen ~r/^the door should stay "(?<status>:[^"]+)" until the 5s timer fires$/, %{status: status}, state do
    assert state.state.door_status == parse_atom(status)
    {:ok, state}
  end

  defthen ~r/^the movement should ONLY begin after the doors are confirmed "(?<status>:[^"]+)"$/, %{status: status}, state do
