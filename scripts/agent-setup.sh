...
#
# Usage: ./agent-setup.sh <agent_name>
...
if [ -z "$1" ]; then
  echo "Usage: ./agent-setup.sh <agent_name>" >&2
  exit 1
fi
...
EXISTING_PORTS=()
for env_file in "$AGENTS_ROOT"/*/.env; do
  if [ -f "$env_file" ]; then
...
if [ ${#EXISTING_PORTS[@]} -eq 0 ]; then
  ...
  # Find max port and increment
  MAX_PORT=$(printf '%s\n' "${EXISTING_PORTS[@]}" | sort -n | tail -1)
  PORT=$((MAX_PORT + 1))
fi
